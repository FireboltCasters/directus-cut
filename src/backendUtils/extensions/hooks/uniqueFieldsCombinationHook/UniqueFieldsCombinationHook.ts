...
export default class UniqueFieldsCombinationHook {
 ...
 static registerHook(data: HookInput) {
   return UniqueFieldsCombinationHook.handleHook.bind(
     null,
     data 
    );
 }
 ...
 static handleHook(
  ...
  async function hookLogic(payload: any, meta: any, {database: any, schema: any, accountability: any}){
    ...
    */
    ...
    //@ts-ignore
    const missingProps = uniqueFields.reduce<string[]>((prev, current) => payloadKeys.includes(current.split(".", 1)[0]) ? prev : [...prev, current.split(".", 1)[0]], [])

    let filledPayloads: Payload[] = []
    if(event === "items.create"){
      filledPayloads = [missingProps.reduce<Payload>((prev, current) => ({...prev, [current]: null}), {...filteredPayload})]
    }
    ...
    if(event === "items.update"){
      ...
      if(missingProps.length > 0){
        const items = await collectionService.readMany(keys, {fields: ["id", ...missingProps]}) as ItemWithId[]
        keys.forEach(itemId => {
            const itemData = items.find(item => item.id == itemId)
            //@ts-ignore
            filledPayloads = [...filledPayloads, missingProps.reduce((prev, current) => ({...prev, [current]: itemData[current]}), {...filteredPayload, id: itemId})]
        });
      }else{
    ...
    */
    const deepRelationExist = uniqueFields.some(field => field.split(".").length > 1)
    ...
    if(deepRelationExist){
      ...
      for(const payloadItem of filledPayloads){
        ...
        for(const field of uniqueFields){
          ...
          let [firstField, ...restFields] = field.split(".")
    ...
    */
    ...
    for(const payloadItem of filledPayloads){
      const duplicatesInCollection = await collectionService.readByQuery({filter: createFilterObject(payloadItem, keys), fields: ["id"]})
...
const hasDuplicates = (payloads: Payload[]) => {
  ...
  return new Set(payloads.map(payload => JSON.stringify(payload))).size !== payloads.length
}

const createFilterObject = (payload: Payload, keys: string[]) => {
  const filterElements = () => {
    return Object.keys(payload).map(field => {
      const conditionPart = payload[field] === null ? { _null: true } : { _eq: payload[field]}
