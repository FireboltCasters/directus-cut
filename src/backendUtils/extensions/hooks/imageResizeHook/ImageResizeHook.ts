...
export default class ImageResizeHook {
  ...
  static async registerHook(beforeHook: any, afterHook: any, collection_name: string, file_field_name: string, payload: any, input: any, database: any, schema: any, accountability: any, registerFunctions: any, context: any) {
    ...
    if(!!imageId){
      ...
      let resizedImageBuffer = await sharp(content).resize(2048, 2048, {fit: 'inside',}).jpeg({ quality: 80 }).toBuffer()
      console.log("Image resized");
      console.log(resizedImageBuffer);

      console.log("upload with fileService");
      try{
        let body = {folder: null, type: "image/png", storage: storageName, filename_download: filename_download}; //TODO get the correct folder
