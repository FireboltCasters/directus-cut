...
export default class ImageBase64UploadHook {
  ...
  static decodeBase64Image(dataString: string){
    ...
    let extension = dataString.substring(dataString.indexOf('/') + 1, dataString.indexOf(';base64'));
  ...
  static async registerHook(beforeHook: any, afterHook: any, collection_name: string, file_field_name: string, payload: any, input: any, database: any, schema: any, accountability: any, registerFunctions: any, context: any){
    ...
    let filesService = new FilesService({schema, accountability: adminAccountAbility});
    let fieldsService = new FieldsService({schema, accountability: adminAccountAbility});
    ...
    if(!!image){
      ...
      if(typeof image === "string" && (image.startsWith("data"))){
        ...
        let body = {folder: null, type: "image/png", storage: storageName, filename_download: "test.png"}; //TODO get the correct folder

        let collectionInfo = await fieldsService.readOne(collection_name, file_field_name);
    ...
    if(!!afterHook){
      payload = await afterHook(collection_name, file_field_name, payload, input, database, schema, accountability, registerFunctions, context);
    }
