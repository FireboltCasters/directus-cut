...
type FieldType =        "alias" | "array" | "binary" | "boolean" | "datetime" | "date" | "time" | "file" |
    "group" | "hash" | "integer" | "decimal" | "json" | "lang" | "m2o" | "o2m" | "slug" | "sort" |
    "status" | "string" | "translation" | "uuid" | "datetime_created" | "datetime_updated" |
...
type PermissionsMap =   { [role: string]: { [collection: string]: { [op in Operation]?: string[] } } };
...
export default class CUTResource<T> {
    ...
    private static get database(): DBImplementation {
        if (!CUTResource.instance) {
            throw Error("[CUTResource] Database implementation not initialized. Use init() first");
        }
    ...
    static initBackend(database: any) {
        console.warn("[CUTResource] Using backend implementation, this is not fully implemented yet");
    ...
    private static defaultComparator = (first: any, second: any) => first === second;
        ...
        for (let data of dataTypes) {
            ...
            for (let type of types) {
                if (!!CUTResource.fieldToTypes[type]) {
                    console.warn("[CUTResource] Conversion function for " + type + " already exists and will be overwritten");
                }
    ...
    private static registerDataTypes() {
        ...
        let dateTimeType: DataType = {
            types: ["datetime", "date", "time", "datetime_created", "datetime_updated"],
            fieldToType: (value: any) => {
                if (typeof value === "string") {
        ...
        CUTResource.registerTypes([booleanType, dateTimeType, integerType, floatType, jsonType]);
    }

    private static async convert(collection: string, obj: any, mode: "toResource" | "toItem"): Promise<any> {
        ...
        for (let key of Object.keys(obj)) {
            ...
            if (value === null) {
                ...
                let caster;
                if (mode === "toResource") {
                    caster = CUTResource.fieldToTypes[type] || CUTResource.defaultFieldToType;
                } else {
                    caster = CUTResource.typeToFields[type] || CUTResource.defaultTypeToField;
                }
    ...
    private static async itemToResource(collection: string, item: any): Promise<any> {
        return await CUTResource.convert(collection, item, "toResource");
    }

    private static async resourceToItem(collection: string, resource: any): Promise<{ [key: string]: string }> {
    ...
    private static async loadMeta() {
        if (!CUTResource.initialized) {
            ...
            for (let data of fieldsData.data) {
                ...
                if (!!foreignKeyTable) {
                    ...
                    if (!CUTResource.relations[data.collection][data.field]) {
                        CUTResource.relations[data.collection][data.field] = foreignKeyTable;
                    }
                }
            }
            ...
            let permissionsData = await CUTResource.database.readMany("directus_permissions");
            for (let data of permissionsData.data) {
                ...
                let fields: string[] = data.fields instanceof Array ? data.fields : [data.fields];
    ...
    private static async filterFieldsByPermission(collection: string, operation: Operation, resourceData: any): Promise<any> {
        await CUTResource.loadMeta();
        ...
        let allowedFields = CUTResource.fieldPermissionsByRole[role][collection][operation] || [];
    ...
    private static areResourcesEqual(collection: string, first: any, second: any): boolean {
        ...
        for (let key of keys) {
            ...
            if (firstValue !== undefined && firstValue !== null) {
                ...
                } else if ((!isFirstArray && isSecondArray) || (isFirstArray && !isSecondArray)) {
                    return false;
                }
                let type = CUTResource.fieldTypes[collection][key];
                let comparator = CUTResource.typeComparisons[type] || CUTResource.defaultComparator;
    ...
    private static async setItem(resource: CUTResource<any>, item: any, associationName: string = resource._collection) {
        resource._itemResource = await CUTResource.itemToResource(associationName, item);
        resource._syncItemResource = await CUTResource.itemToResource(associationName, item);
    }

    // public api

    static async getAll<T>(collection: string, offset: number | null, limit: number | null, multiSortMeta: any | null, filterParams: object | null): Promise<CUTResource<T>[]> {
    ...
    static async create<T>(collection: string, resourceData: any): Promise<CUTResource<T> | null> {
        let resource = new CUTResource<T>(collection);
        let filtered = await CUTResource.filterFieldsByPermission(collection, "create", resourceData);
    ...
    isSynchronized(): boolean {
        ...
        return CUTResource.areResourcesEqual(this._collection, this._itemResource, this._syncItemResource);
    }

    private async getOneToMany<T>(associationTableName: string, filterParams?: any): Promise<CUTResource<T>[]> {
        let res: CUTResource<T>[] = [];
        ...
        let associations = await CUTResource.database.readMany(associationTableName, {
            filter: filter
        });
        if (!!associations) {
    ...
    private async getManyToMany<T>(junctionTables: JunctionData[], associationTableName: string, filterParams?: any): Promise<CUTResource<T>[]> {
        let res: CUTResource<T>[] = [];
        ...
        for (let junctionTable of junctionTables) {
            ...
            let junctionTableKeyAssociation = junctionTable.foreignKeys[associationTableName];
            let junctionAssociations = await CUTResource.database.readMany(junctionTableName, {
    ...
    async getAssociations<T>(associationTableName: string, filterParams?: any): Promise<CUTResource<T>[]> {
        await CUTResource.loadMeta();
        let junctionTables = CUTResource.junctionTables[this._collection + ":" + associationTableName];
        ...
        return await this.getManyToMany(junctionTables, associationTableName, filterParams);
    }

    private async addOneToMany<T>(associationTableName: string, associationResource: CUTResource<T>): Promise<void> {
    ...
    private async addManyToMany<T>(associationTableName: string, associationResource: CUTResource<T>): Promise<void> {
        let relation = CUTResource.relations[associationTableName];
        let thisKey = "";
        let associationKey = "";
        for (let key of Object.keys(relation)) {
    ...
    async addAssociation<T>(associationResource: CUTResource<T>, associationTableName?: string): Promise<void> {
        await CUTResource.loadMeta();
        if (!associationTableName) {
            await this.addOneToMany(associationResource._collection, associationResource);
        } else {
    ...
    async save() {
        let filtered = await CUTResource.filterFieldsByPermission(this._collection, "update", this._itemResource);
        let data = await CUTResource.resourceToItem(this._collection, filtered);
        let updatedItem = await CUTResource.database.updateOne(this._collection, this._itemResource.id, data);
        await CUTResource.setItem(this, updatedItem);
    }

    async delete(): Promise<boolean> {
        return await CUTResource.database.deleteOne(this._collection, this._itemResource.id);
    }
